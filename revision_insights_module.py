    ...
    for attempt in range(MAX_LLM_RETRIES + 1):
        try:
            ...
            if attempt == MAX_LLM_RETRIES:
                print("Failed to extract valid insights from revision history after multiple retries.", file=sys.stderr)
                return False
            insights_extraction_prompt += "\nYour previous response was not a valid JSON or did not match the schema. Please try again."
        except Exception as e_gen:
            print(f"Unexpected error during insights extraction (attempt {attempt + 1}): {e_gen}", file=sys.stderr)
            if attempt == MAX_LLM_RETRIES:
                return False
