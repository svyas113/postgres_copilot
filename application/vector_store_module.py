...
def _initialize_embedding_model() -> SentenceTransformer: # Removed model_name argument
    ...
    if embedding_model is None:
        try:
            # Find the data directory and create cache subdirectory
            data_dir = find_data_directory()
            cache_dir = data_dir / "cache" / "sentence_transformers"
...
def _get_lancedb_base_uri() -> Path:
    """Gets the base URI for LanceDB stores by finding the data directory dynamically."""
    global _lancedb_base_uri_cache
    if _lancedb_base_uri_cache is None:
        # Find the data directory and create lancedb_stores subdirectory
        data_dir = find_data_directory()
        lancedb_path = data_dir / "memory" / "lancedb_stores"
        lancedb_path.mkdir(parents=True, exist_ok=True)
        _lancedb_base_uri_cache = lancedb_path
...
def get_table_name(db_name_identifier) -> str:
    ...
    return "".join(c if c.isalnum() or c == '_' else '_' for c in db_name_str)
...
if __name__ == "__main__":
    ...
    # Find the data directory and ensure subdirectories exist
    data_dir = find_data_directory()
    print(f"Using data directory: {data_dir}")
    
    # Ensure directories exist
    (data_dir / "memory" / "lancedb_stores").mkdir(parents=True, exist_ok=True)
    (data_dir / "cache" / "sentence_transformers").mkdir(parents=True, exist_ok=True)
